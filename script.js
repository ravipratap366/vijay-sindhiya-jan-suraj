...
const realVoterDataSample = [
    ...
    },
    {
        ...
        father_name: "नंद किशोर पासवान", father_name_en: "Nand Kishor Paswan", age: 23, gender: "M",
        address: "गांव - प्रेमनगर सुहई", address_en: "Village - Premnagar Suhai",
        tehsil: "रुन्नीसैदपुर", tehsil_en: "Runnisaidpur",
        post_office: "प्रेमनगर सुहई", post_office_en: "Premnagar Suhai",
        booth: "001", house_no: "00"
    ...
    {
        serial: 11, voter_id: "WHD3847592", name: "रवि प्रताप", name_en: "Ravi Pratap",
        father_name: "राम प्रकाश", father_name_en: "Ram Prakash", age: 32, gender: "M",
        address: "गांव - बघाड़ी", address_en: "Village - Baghadi",
        tehsil: "रुन्नीसैदपुर", tehsil_en: "Runnisaidpur",
        post_office: "बघाड़ी", post_office_en: "Baghadi",
        booth: "002", house_no: "15"
    },
    {
        serial: 12, voter_id: "WHD2847193", name: "प्रिया सिंह", name_en: "Priya Singh",
        father_name: "अजय सिंह", father_name_en: "Ajay Singh", age: 28, gender: "F",
        address: "गांव - मानिक चौक", address_en: "Village - Manik Chauk",
        tehsil: "रुन्नीसैदपुर", tehsil_en: "Runnisaidpur",
        post_office: "मानिक चौक", post_office_en: "Manik Chauk",
        booth: "005", house_no: "22"
    },
    {
        serial: 13, voter_id: "WHD2947583", name: "अनिल कुमार", name_en: "Anil Kumar",
        father_name: "लाल बहादुर", father_name_en: "Lal Bahadur", age: 35, gender: "M",
        address: "गांव - गौड़ा", address_en: "Village - Gauda",
        tehsil: "रुन्नीसैदपुर", tehsil_en: "Runnisaidpur",
        post_office: "गौड़ा", post_office_en: "Gauda",
        booth: "003", house_no: "08"
    },
    {
        serial: 14, voter_id: "WHD2637481", name: "सुमित्रा देवी", name_en: "Sumitra Devi",
        father_name: "गोपाल साह", father_name_en: "Gopal Sah", age: 45, gender: "F",
        address: "गांव - अथरी", address_en: "Village - Athari",
        tehsil: "रुन्नीसैदपुर", tehsil_en: "Runnisaidpur",
        post_office: "अथरी", post_office_en: "Athari",
        booth: "010", house_no: "12"
    },
    {
        serial: 15, voter_id: "WHD2745629", name: "राहुल मिश्रा", name_en: "Rahul Mishra",
        father_name: "विनोद मिश्रा", father_name_en: "Vinod Mishra", age: 26, gender: "M",
        address: "गांव - रायपुर", address_en: "Village - Raipur",
        tehsil: "रुन्नीसैदपुर", tehsil_en: "Runnisaidpur",
        post_office: "रायपुर", post_office_en: "Raipur",
        booth: "015", house_no: "05"
    },
    {
        serial: 16, voter_id: "WHD2139061", name: "रवि प्रताप", name_en: "Ravi Pratap",
        father_name: "विजय कुमार साह", father_name_en: "Vijay Kumar Sah", age: 26, gender: "M",
        address: "रुन्नीसैदपुर", address_en: "Runnisaidpur",
        tehsil: "रुन्नीसैदपुर", tehsil_en: "Runnisaidpur",
        post_office: "रून्नीसैदपुर", post_office_en: "Runnisaidpur",
        booth: "270", house_no: "रुनीसैदपुर"
    }
];
...
async function initializeChunkedData() {
    try {
        ...
        console.log(`Loaded chunk index: ${voterChunkIndex.metadata.total_chunks} chunks, ${voterChunkIndex.metadata.total_records} total records`);
        
        // Update UI with actual statistics
        const totalVotersEl = document.getElementById('totalVoters');
        if (totalVotersEl) {
            totalVotersEl.textContent = voterChunkIndex.metadata.total_records.toLocaleString('hi-IN');
        }
...
async function loadChunk(chunkId) {
    ...
    try {
        const response = await fetch(`voter-chunks/chunk-${chunkId.toString().padStart(3, '0')}.json`);
        if (!response.ok) {
...
async function performChunkedSearch(searchTerm) {
    ...
    if (resultsDiv) {
        resultsDiv.innerHTML = '<div class="loading">🔍 खोज रहे हैं... कृपया प्रतीक्षा करें</div>';
    }
    ...
    try {
        ...
        for (let i = 0; i < voterChunkIndex.metadata.total_chunks; i++) {
            ...
            if (chunksProcessed % 3 === 0) {
                const progress = Math.round((chunksProcessed / voterChunkIndex.metadata.total_chunks) * 100);
                if (resultsDiv) {
                    resultsDiv.innerHTML = `<div class="loading">🔍 खोज रहे हैं... ${progress}% पूर्ण (${exactMatches.length + goodMatches.length + partialMatches.length} मिले)</div>`;
                }
            }
            
            // Early termination conditions
            const totalFound = exactMatches.length + goodMatches.length + partialMatches.length;
            
            // If we have enough exact matches, stop searching
            if (exactMatches.length >= MAX_RESULTS) {
                console.log(`Found ${exactMatches.length} exact matches, stopping search early`);
                break;
            }
            
            // If we have a good number of high-quality results and processed significant chunks
            if (totalFound >= MAX_RESULTS * 2 && chunksProcessed >= Math.min(20, voterChunkIndex.metadata.total_chunks * 0.4)) {
                console.log(`Found ${totalFound} results after processing ${chunksProcessed} chunks, stopping search`);
                break;
            }
        }
...
function calculateRelevanceScore(voter, searchTerm) {
    ...
    else if (searchWords.length > 1 && searchWords.every(word => name.includes(word))) {
        score += 65;
    }
...
function searchInVoterRecord(voter, searchTerm) {
    ...
    return searchFields.some(field => {
        ...
        const fieldLower = field.toString().toLowerCase().trim().replace(/\s+/g, ' ');
        ...
        if (fieldNoSpaces.includes(termNoSpaces) && termNoSpaces.length > 2) return true;
...
function initializeRealData() {
    ...
    displayRealDataInfo();
    console.log('Real voter data loaded: 15 sample records from 279,729 total voters');
}
...
function displayRealDataInfo() {
    ...
    const existingNote = document.querySelector('.simple-note, .data-note, .data-info-section');
...
function universalSearch(searchTerm) {
    ...
    const rankedResults = voterData.map(voter => {
...
function calculateSampleDataRelevance(voter, searchTerm) {
    ...
    else if (searchWords.every(word => name.includes(word) || nameEn.includes(word))) {
        score += 60;
    }
    ...
    for (const word of searchWords) {
        ...
        if (translations) {
            for (const translation of translations) {
                ...
                if (normalizedField.includes(normalizedTranslation) || 
                    normalizedField.replace(/\s+/g, '').includes(normalizedTranslation.replace(/\s+/g, ''))) {
                    return true;
                }
            }
        }
...
function displaySearchResults(results, searchTerm) {
    ...
    const resultsHTML = resultsHeader + results.map((voter, index) => `
...
const pollingStationData = [
    ...
    {station: "राजकीया प्राथमिक विधालय कन्या प्रेमनगर गोट के प्रांगन मे चलंत मतदान", count: 3387},
    ...
    {station: "मौजे किशोरी बालिका उच्चतर माध्यमिक विद्यालय मानिक चौक", count: 2085},
    ...
    {station: "राजकीय प्राथमिक विद्यालय मानिक चौक (अनुसूचित जाति टोला)", count: 1764},
    ...
    {station: "राजकीय प्राथमिक विद्यालय, मानिक चौक टोले हरसिंगपुर", count: 1557},
    ...
    {station: "राजकीय प्राथमिक बालक विद्यालय, प्रेमनगर बहुरी टोला", count: 1355},
    ...
    {station: "अनुसुचित जाति दलान, बाथ असली राम अधीन पासवान के घर के निकट", count: 1146},
    ...
    {station: "राजकीय प्राथमिक विद्यालय, पोता उर्फ तिलक ताजपुर पूर्वी टोला", count: 1041},
    ...
    {station: "प्राथमिक विद्यालय, बुलंदपुर उर्फ हुसैनपुर बलान टोला", count: 935},
...
function initPollingAnalytics() {
    if (typeof Plotly !== 'undefined' && document.getElementById('polling-chart')) {
        showChart('bar');
    }
}
...
function showBarChart() {
    ...
    const trace = {
        ...
        y: allStations.map(d => d.station.length > 50 ? d.station.substring(0, 50) + '...' : d.station),
        type: 'bar',
        orientation: 'h',
        marker: {
...
function showPieChart() {
    ...
    const labels = [...top10.map(d => d.station.length > 30 ? d.station.substring(0, 30) + '...' : d.station), 'अन्य केंद्र'];
    ...
    const trace = {
        ...
        type: 'pie',
        marker: {
            colors: ['#FF6B35', '#F7931E', '#FFE66D', '#27AE60', '#3498DB', '#9B59B6', '#E74C3C', '#95A5A6', '#F39C12', '#2ECC71', '#BDC3C7']
        },
...
async function showAgeDistributionChart() {
    ...
    if (voterChunkIndex) {
        ...
        for (let i = 0; i < Math.min(10, voterChunkIndex.metadata.total_chunks); i++) { // Limit to first 10 chunks for performance
            try {
                const chunkData = await loadChunk(i);
    ...
    for (let age = Math.floor(minAge / binSize) * binSize; age <= maxAge; age += binSize) {
        bins.push(age);
    ...
    const trace = {
        ...
        hovertemplate: '<b>आयु समूह: %{x}</b><br>' +
                      'मतदाता संख्या: %{y}<br>' +
                      '<extra></extra>'
    };
    ...
    const layout = {
        title: {
            text: `👥 मतदाताओं का आयु वितरण (${allAges.length.toLocaleString('hi-IN')} नमूना)`,
            font: {size: 16, color: '#2C3E50', family: 'Poppins'}
        },
        xaxis: {
            title: 'आयु समूह (वर्ष)',
